...
impl WindowsFirewall {
    ...
    fn get_rule_full_info(
        &self,
        name: &str,
        direction: &str,
    ) -> Result<RuleFullInfo, FirewallError> {
        let output = self.run_netsh(&[
            "advfirewall",
            "firewall",
            "show",
        ...
        for line in output.lines() {
            ...
            if key_lower.contains("enabled") || key_lower.contains("已启用") {
                info.enabled =
                    val_lower == "yes" || val == "是" || val_lower == "true" || val_lower == "1";
            } else if key_lower.contains("action")
                || key_lower.contains("操作")
                || key_lower.contains("动作")
            {
                info.action =
                    if val_lower.contains("block") || val.contains("阻止") || val.contains("拒绝")
                    {
                        "block".to_string()
                    } else {
                        "allow".to_string()
...
impl WindowsFirewall {
    ...
    pub fn update_rule(
        ...
        let eff_action = match action {
            ...
            _ => {
                if old_rule.action.is_empty() {
                    "allow".to_string()
                } else {
                    old_rule.action.clone()
                }
            }
        };
        let eff_protocol = match protocol {
            ...
            _ => {
                if old_rule.protocol.is_empty() {
                    "any".to_string()
                } else {
                    old_rule.protocol.clone()
                }
            }
        };
        ...
        let eff_local_addr: Option<String> = match local_addr {
            ...
            None => {
                if self.is_any_value(&old_rule.local_ip) {
                    None
                } else {
                    Some(old_rule.local_ip.clone())
                }
            }
        };
        let eff_description: Option<String> = match description {
            ...
            None => {
                if old_rule.description.is_empty() {
                    None
                } else {
                    Some(old_rule.description.clone())
                }
            }
        };
